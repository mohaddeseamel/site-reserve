<template>
  <div
    class="register-form-account lg:bg-green-light bg-white-default w-screen h-screen flex items-center justify-center"
  >
    <div
      class="formRegister-account flex items-center justify-center bg-white-default container mx-auto p-10 rounded-3xl relative"
    >
      <div class="flex absolute top-0 lg:right-0 left-0 p-14">
        <svg
        ...
        <span class="text-green-dark lg:text-2xl text-lg font-[500]"
          >Colorful Beauty</span
        >
      </div>
      <div class="flex flex-col items-center justify-center lg:w-1/2 w-full">
        <div
          class="form-account bg-gray-secondgray lg:w-2/3 w-full h-full p-10 leading-10 rounded-lg"
        >
          <h1 class="text-start mb-12 text-[1.5rem]">ثبت نام کنید</h1>
          <form action="#">
            <div class="name flex flex-col mb-2">
              <label for="name" class="text-[13px] text-darken"
                >نام و نام خانوادگی</label
              >
              <div class="mobile-input relative bg-white-default rounded-lg">
                <Icon
                  icon="mdi:user"
                  class="absolute left-2 top-3 text-gray-light"
                />
                <input
                  type="text"
                  placeholder=""
                  class="outline-none px-3 ml-5 text-left w-11/12"
                  v-model="formDataRegister.name"
                />
              </div>
            </div>
            <div class="phoneNumber flex flex-col mb-2">
              <label for="number" class="text-[13px] text-darken"
                >شماره تلفن</label
              >
              <div class="mobile-input relative bg-white-default rounded-lg">
                <Icon
            ...
            <div class="password flex flex-col">
              <label for="password" class="text-[13px]">رمز عبور</label>
              <div class="mobile-input relative bg-white-default rounded-lg">
                <Icon
                ...
                <input
                  type="text"
                  placeholder=""
                  class="bg-white-default outline-none px-3 text-left w-11/12"
                  v-model="formDataRegister.password"
                />
              </div>
            </div>
            <button
              class="w-full bg-green-dark text-white-default rounded-lg mt-5"
            >
              ثبت نام
            </button>
          </form>
          <div class="create-account flex justify-center mt-3 text-[13px]">
            ...
            <router-link
              to="/AccountUser"
              class="text-green-dark cursor-pointer"
              >وارد شوید</router-link
            >
          </div>
        </div>
      </div>
      <div class="imgAccount lg:w-1/2 w-0">
        <img src="../assets/imgs/beauty-2.jpg" alt="accoutImg" />
      </div>
    </div>
  </div>
</template>
